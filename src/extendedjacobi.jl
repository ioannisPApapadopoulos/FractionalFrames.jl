...
function diff(P::ExtendedJacobi{T}; dims=1) where T
    DerivativeExtendedJacobi{T}(P.a, P.b)
end
...
function diff(P::ExtendedWeightedJacobi{T}; dims=1) where T
    DerivativeExtendedWeightedJacobi{T}(P.a, P.b)
end
...
function _coeff(s, m)
    n = m ÷ 2
    if iseven(m)
        return 4^s * gamma(1 + s + n)*gamma(s + 1/2 + n) / (gamma(1 + n) * gamma(n + 1/2))
    else
        return 4^s * gamma.(1 + s + n)*gamma(s + 3/2 + n) / (gamma(1 + n) * gamma(n + 3/2))
    end
end
...
function *(L::AbsLaplacianPower, P::ExtendedJacobi{T}) where T
    ...
    s = P.a
    ExtendedWeightedJacobi{T}(s,s) .* (one(T) ./ _coeff.(s, 0:∞))'
end

function *(L::AbsLaplacianPower, Q::ExtendedWeightedJacobi{T}) where T
    @assert axes(L,1) == axes(Q,1)

    Q.a == Q.b == L.α && return ExtendedJacobi{T}(Q.a, Q.a) .* _coeff.(Q.a, 0:∞)'
