...
recurrencecoefficients(P::ExtendedChebyshevT{T}) where T = recurrencecoefficients(ChebyshevT{T}())
recurrencecoefficients(P::ExtendedChebyshevU{T}) where T = recurrencecoefficients(ChebyshevU{T}())
...
recurrencecoefficients(wT::ExtendedWeightedChebyshevT{T}) where T = recurrencecoefficients(ChebyshevT{T}())
recurrencecoefficients(wU::ExtendedWeightedChebyshevU{T}) where T = recurrencecoefficients(ChebyshevU{T}())
...
function diff(T̃::ExtendedChebyshevT{T}; dims=1) where T
    D = BandedMatrix(-1=>zero(T):∞)
    ApplyQuasiMatrix(*, ExtendedChebyshevU{T}(), D)
end

function diff(W::ExtendedWeightedChebyshevU{T}; dims=1) where T
    D = BandedMatrix(-1=>-one(T):-one(T):∞)
    ApplyQuasiMatrix(*, ExtendedWeightedChebyshevT{T}(), D)
end
